---
title: "Peer-to-peer vs authoritative multiplayer networking"
---
...
<Note>
    ...
    We built this guide with the intent of helping everyone choose how to build their multiplayer game based on years of experience -- regardless of if they use Rivet. If you believe there is an inaccurate or unfair comparison, please [reach out](https://discord.gg/BG2vqsJczH) or [make a PR](https://github.com/rivet-gg/docs).
</Note>
...
There are other lesser-used models of peer-to-peer networking that don't require one player to act as the server, but we'll keep it simple for this guide.

Some notable peer-to-peer games include:

- GTA V & RDR 2

### 🤖 Authoritative

Authoritative multiplayer consists of a server in a data center hosting the master copy of the game state and players connecting to that host.

Some notable authoritative games include:
...
Worst yet -- some ISPs don't have peering agreements with each other. That means that if player 1 is on ISP A and player 2 is on ISP B, traffic may need to be routed to a [higher tier](https://en.wikipedia.org/wiki/Tier_1_network) exchange point which is usually not close to the player. For example, traffic from players 1 to 2 may be routed from Phoenix -> Los Angelos -> Phoenix just because they're using different ISPs.
...
However, saving money here limits the reach and leads to a worse gaming experience compared to authoritative networking.

<Note>
    Rivet provides the ability to auto-scale your lobbies and pay based on usage to optimize cost.
</Note>
