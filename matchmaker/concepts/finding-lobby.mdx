...
## A note on competitive matchmaking
...
If this is not the desired behavior, [`Lobbies.find`](/matchmaker/api/post-lobbies-find) can be called with `prevent_auto_create_lobby` as `true`. This will return a [`MATCHMAKER_NO_AVAILABLE_LOBBIES`](/error/mm/no-available-lobbies) error when no lobby can accept players.
